...
Now let's calculate the annual GDP growth rates for France:
...
Similarly, for quarterly GDP, we can calcualte the year-on-year and quarter-on-quarter annualized growth rates:

```{r, message=FALSE, warning=FALSE}
#year-on-year rate:
g_yoy = (france/lag(france,k=4)-1)*100 #lag 4 quarters
#quarter-on-quarter annualized rate:
g_qoq = ((france/lag(france))^4-1)*100 #lag 1 quarter
cbind(g_yoy,g_qoq)

#Since g_yoy is only available from 2001.Q1, we plot both series from 2001.Q1 to 2002.Q4
t = "2001-01-01/2002-10-01"
g_yoy = g_yoy[t]
g_qoq = g_qoq[t]
data = cbind(g_yoy,g_qoq)
colnames(data) <- c('YoY Rate','QoQ Annualized Rate')
plot(data,legend.loc='topright',auto.legend=TRUE,main='French GDP Growth Rate')
```
